# Diagonal BP-GAS-FZ: one-step-ahead OOS forecasts for VaR/ES and per-period FZ0 loss
BP_GAS_FZ_Diag_Forecast <- function(theta_estimated,
                                    Y1_train, Y2_train,
                                    Y1_oos, Y2_oos,
                                    w1, w2,
                                    alpha,
  ...
  # Parameter mapping (same as in estimation objective)
  ...
  alpha1  <- -exp(theta_estimated[6])
  alpha2  <- -exp(theta_estimated[7])
  alpha12 <- -exp(theta_estimated[8])
